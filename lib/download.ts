...
export async function download(projectId: string, languages: string[]) {
  const translationResponse = await supabase
    .from("translatedKeys")
    .select("keys(id, name), translations")
  ...
  translationResponse.data?.forEach((e: any) => {
    languages.forEach((langCode) => {
      if (translations[`${langCode}`] == null) {
        translations[`${langCode}`] = {};
      }
      translations[`${langCode}`][`${e.keys?.name}`] =
        `${e.translations[langCode]}`;
    });
  });
